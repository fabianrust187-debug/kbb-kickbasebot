...
const INJURY_SETTLE_MS = Math.max(10_000, Number(process.env.KBB_INJURY_SETTLE_MS || 20_000));
const MARKER = "KBBLIVE5:";
const OLD_MARKERS = ["KBBLIVE4:", "KBBLIVE3:", "KBBLIVE2:", "KBBLIVE:", "KBBGOAL:"];
const RED_POINTS = -75;
...
function cardActions(summary, event) {
  ...
  for (const entry of allEntries(summary, event)) {
    ...
    const m = minute(entry.item?.clock?.displayValue ?? entry.item?.clock);
    const identity = `${kind}|${normalize(playerName)}`;
    const candidate = {
      entry,
      kind,
      playerName,
      ...
      quality: entryScore(entry) + (m.valid ? 100 : 0),
    };
...
function injuryActions(summary, event) {
  ...
  for (const entry of allEntries(summary, event)) {
    ...
    const m = minute(entry.item?.clock?.displayValue ?? entry.item?.clock);
    // A player can only have one injury-forced substitution in the same match.
    // ESPN may expose it once without a clock and again with the real minute.
    // Therefore the semantic identity intentionally excludes the minute.
    const identity = normalize(playerName);
    const candidate = {
      entry,
      playerName,
      m,
...
function preferRicherInjury(oldAction, newAction) {
  if (!oldAction) return newAction;
  const oldMinute = minute(oldAction.minute);
  const newMinute = minute(newAction.minute);
  if (!oldMinute.valid && newMinute.valid) return newAction;
  if (oldMinute.valid && !newMinute.valid) return oldAction;
  return newAction;
}
...
async function processGuild(guild) {
  ...
  try {
    ...
    if (!state.hydrated) {
      ...
      state.hydrated = true;
      console.log(`⚽ Bundesliga V5 recovery: ${state.seen.size} marker(s)`);
    }
    ...
    for (const event of games) {
      ...
      const summary = await fetchJson(summaryUrl(eventId)).catch(error => {
        console.warn(`⚠️ Bundesliga V5 summary ${eventId}: ${error?.message || error}`);
        return null;
      });
      if (!summary) continue;
      ...
      if (firstObservation) {
        ...
        console.log(`🛡️ Bundesliga V5 baseline ${eventId}: ${current.length} existing event(s)`);
        continue;
      }
      ...
      for (const action of current) {
        if (state.seen.has(action.key)) {
          ...
          state.pendingInjuries.delete(action.key);
          continue;
        }
        ...
        if (action.kind === "injury") {
          const pending = state.pendingInjuries.get(action.key);
          if (!pending) {
            state.pendingInjuries.set(action.key, {
              firstSeenAt: Date.now(),
              event,
              action,
            });
          ...
          pending.event = event;
          pending.action = preferRicherInjury(pending.action, action);
          const settled = Date.now() - pending.firstSeenAt >= INJURY_SETTLE_MS || isCompleted(event);
          if (settled) ready.push({ event: pending.event, action: pending.action });
          continue;
        }
    ...
    for (const { event, action } of ready) {
      ...
      const sent = await channel.send({
        ...
        console.error(`❌ Bundesliga V5 post failed: ${error?.message || error}`);
        return null;
      });
      if (!sent) continue;
      ...
      state.pendingInjuries.delete(action.key);
      console.log([
        `✅ Bundesliga V5 ${action.kind}: ${action.playerName}`,
    ...
    console.error(`❌ Bundesliga V5 poll failed for ${guild.id}:`, error?.message || error);
  } finally {
    runningGuilds.delete(guild.id);
  }
}

export function startBundesligaLiveFeedSchedulerV5(client) {
  const run = async () => {
    for (const guild of client.guilds.cache.values()) await processGuild(guild);
  };

  setTimeout(() => run().catch(() => null), 8_000);
  console.log(`⚽ Bundesliga live feed V5 ready: channel=${CHANNEL_ID}, interval=${POLL_MS}ms, goal-settle=${GOAL_SETTLE_MS}ms, injury-settle=${INJURY_SETTLE_MS}ms`);
